...
#     answer = 0
#     #answer라는 이름의 변수를 만들고, 이를 0으로 초기화합니다. 이 변수는 약수의 합을 저장할 공간입니다.
#     # 1부터 n까지의 숫자를 순회합니다.
...
def solution(n):
    ...
    for i in range(1, n + 1):
        if n % i == 0:
            answer.append(i)
            answer_sum += i
    return answer,answer_sum

user_input = input("put: ")
num = int(user_input)
answer, answer_sum = solution(num)
print(f"약수 리스트는 {answer}이고")
print(f"내가 원하는 약수의 합은 {answer_sum}이였어")